...
if(create_database($dbname, $password, $optcmd)) {
 ...
 my $dbhomelocation=`$ODACLI describe-dbhome -i $dbhomeid|grep -i "location"`;
 my @location=split /:\s*/,$dbhomelocation;
 my $dbhome=$location[1];
 chomp $dbhome;
...
sub if_flash{
	my $tmp=`/opt/oracle/oak/bin/odaadmcli show diskgroup|grep -i FLASH`;
	if($tmp){
	return 1;
	}else{
	return 0;
