...
if(!$ARGV[0]=~/160419/){
 $choose_co=&choose_co;
my $t=time();
print "create 11204 database-1 successfully!Time is:".(time()-$t)."\n" if(create_database("my11db1", $db_password, "-cl OLTP -dh $Ora11gDBhome[0] $choose_co"));
 $choose_co=&choose_co;
 $t=time();
print "create 11204 database-2 successfully!Time is:".(time()-$t)."\n" if(create_database("my11db2", $db_password, "-cl OLTP -dh $Ora11gDBhome[0] $choose_co"));
 $choose_co=&choose_co;
 $t=time();
print "create 11204 database-3 successfully!Time is:".(time()-$t)."\n" if(create_database("my11db3", $db_password, "-cl OLTP -dh $Ora11gDBhome[0] $choose_co"));
} 
$choose_co=&choose_co;
 $choose_c=&choose_c;
 $t=time();
print "create 12102 database-1 successfully!Time is:".(time()-$t)."\n" if(create_database("my12db1", $db_password, "-cl OLTP -dh $Ora12gDBhome[0] -r ASM $choose_c $choose_co"));
 $choose_co=&choose_co;
 $choose_c=&choose_c;
 $t=time();
print "create 12102 database-2 successfully!Time is:".(time()-$t)."\n" if(create_database("my12db2", $db_password, "-cl OLTP -dh $Ora12gDBhome[0] -r ACFS $choose_c $choose_co"));
 $choose_co=&choose_co;
 $choose_c=&choose_c;
 $t=time();
print "create 12102 database-3 successfully!Time is:".(time()-$t)."\n" if(create_database("my12db3", $db_password, "-cl OLTP -dh $Ora12gDBhome[1] -r ASM $choose_c $choose_co"));
 $choose_co=&choose_co;
 $choose_c=&choose_c;
 $t=time();
print "create 12102 database-4 successfully!Time is:".(time()-$t)."\n" if(create_database("my12db4", $db_password, "-cl OLTP -dh $Ora12gDBhome[1] -r ACFS $choose_c $choose_co"));
$choose_co=&choose_co;
 $choose_c=&choose_c;
 $t=time();
print "create 12102 database-5 successfully!Time is:".(time()-$t)."\n" if(create_database("my12db5", $db_password, "-cl OLTP -dh $Ora12gDBhome[0] -r ACFS $choose_c $choose_co"));
$choose_co=&choose_co;
 $choose_c=&choose_c;
 $t=time();
print "create 12102 database-6 successfully!Time is:".(time()-$t)."\n" if(create_database("my12db6", $db_password, "-cl OLTP -dh $Ora12gDBhome[1] -r ACFS $choose_c $choose_co"));
}
