...
print "create 12.1.0.2.171017 db successfully!\n" if(create_database("new1", $db_password, "-v 12.1.0.2.171017 -r acfs -s odb1s"));
@temp=&list_databases;
my $database_id=shift @temp;
print "upgrade a new 12.1 db to the new created dbhome successfully!\n" if(upgrade_database($database_id,$dbhome_id));
...
&need_scp("12.1.0.2.170814");
print "create 12.1.0.2.170814 db successfully!\n" if(create_database("new2", $db_password, "-v 12.1.0.2.170814 -r asm -s odb1s"));
@temp=&list_databases;
$database_id=shift @temp;
print "upgrade a new lower 12.1 db to the new created dbhome successfully!\n" if(upgrade_database($database_id,$dbhome_id));

&need_scp("11.2.0.4.171017");
print "create 11.2.0.4.171017 db successfully!\n" if(create_database("new3", $db_password, "-v 11.2.0.4.171017 -r acfs"));
@temp=&list_databases;
$database_id=shift @temp;
my $dbhome_id_121=`odacli list-dbhomes|grep OraDB12102_home1|awk '{print \$1}'`;
chomp $dbhome_id_121;
print "upgrade a new 11.2 db to a 12.1 dbhome successfully!\n" if(upgrade_database($database_id,$dbhome_id_121));
